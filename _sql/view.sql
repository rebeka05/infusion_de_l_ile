...
-- select datevente, sum(total) as total
-- from v_etat_vente_mensuel_entite
-- where idmois=1 and annee='2024' and idclient=1
-- group by datevente

-- select c.idclient, coalesce(extract(month from v.datevente), m.idmois) as idmois, m.mois, coalesce( sum(total), 0) as total
-- from client c cross join mois m
-- left join v_vente_valide v on v.idclient=c.idclient
-- where m.idmois=extract(month from v.datevente)
-- group by c.idclient, coalesce(extract(month from v.datevente), m.idmois), m.mois
-- order by idmois;
